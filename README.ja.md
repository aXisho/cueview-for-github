...
ページ名が `.gloss.md` または `.gloss` で終わる Wiki ページ（例: `/owner/repo/wiki/Page-Name.gloss.md`）を自動検出し、`raw.githubusercontent.com/wiki/` から生コンテンツを取得して描画します。
...
GitHub Wiki ページ (/wiki/ URL、ページ名が .gloss.md / .gloss で終わる)
  └─ content.ts が document_idle で実行
       ├─ isGlossMdPath() → true
